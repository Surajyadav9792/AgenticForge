...
export async function blueprintValidatorNode(state) {
  ...
  if (dbSchema?.tables && apiEndpoints) {
    ...
    for (const table of dbSchema.tables) {
      ...
      const hasFKs = table.foreignKeys && table.foreignKeys.length >= 2;
      const hasBusinessFields = (table.fields || []).some(f => {
        const fname = f.name.toLowerCase();
        // Ignore ID, foreign key columns, and timestamps
        if (fname === "id" || fname === "created_at" || fname === "updated_at") return false;
        if (table.foreignKeys?.some(fk => fk.field?.toLowerCase() === fname)) return false;
        return true;
      });
      const isJunction = hasFKs && !hasBusinessFields;
