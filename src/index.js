...
async function main() {
  ...
  try {
    ...
    } else if (error.message?.includes("API_RATE_LIMIT_EXCEEDED")) {
      console.error("\n  🛑 API Rate Limit Exceeded!");
      console.error(`     Reason: ${error.message}`);
      console.error("     The run could not complete because the model provider rate limits or quotas were hit.");
      console.error(`     You can try to resume this run later using:\n     node src/index.js --resume ${threadId}`);
    } else {
      console.error("\n  ❌ Error:", error.message);
