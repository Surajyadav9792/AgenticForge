...
async function _executeGraph(projectId, graph, config, requirement, tokenBudget, emit) {
  ...
  try {
    ...
    if (error.name === "AbortError") {
      ...
      const isRateLimit = error.message?.includes("API_RATE_LIMIT_EXCEEDED");
      const isBudgetExceeded = error.message?.includes("TOKEN_BUDGET_EXCEEDED");
      let errorCode = "GENERIC_ERROR";
      if (isRateLimit) errorCode = "API_RATE_LIMIT_EXCEEDED";
      else if (isBudgetExceeded) errorCode = "TOKEN_BUDGET_EXCEEDED";
